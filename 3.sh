...
if [ ! -f "$prefix/lib/libX11.so" ] || [ ! -f "$prefix/lib/libandroid-shmem.so" ]; then
  ...
  fetchDeb "libxcursor" "libx/libxcursor"          # 修复语法错误
...
# 收集依赖库（需要重新声明 pendingElfs 在 collectLib 的上下文中）
# 由于 collectLib 内部使用全局 pendingElfs，这里需要确保在 collectLib 中能访问
# 使用子 shell 来执行 collectLib 会导致 pendingElfs 丢失，因此直接调用
